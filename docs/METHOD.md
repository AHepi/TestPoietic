...
The campaign is frozen in the first Git commit and the immutable
`stress-plan-v1` tag before execution. `evidence/frozen/stress-plan-v1.json`
contains the exact expected populations and outcomes. Campaign output is
deterministic JSON. The repository verifier reruns every test in a temporary
directory and compares every stored evidence file by SHA-256.
...
| Formal models | validity of a universal implication under the encoded definitions | actual physical possibility unless separately established |
...
The W3 model is repaired by supplying candidate/occupant identity, current
criterion licensing, and open permission. P3.1 is enumerated both with its
full identity-protection hypothesis and with the historical weakened mutant.
